...
- The alternative solution is to use custom escape sequences, like `+#SVMD0;` perhaps. These would only need to escape invalid `<` and `{` characters, and then escaping preprocess would only be responsible for strictly escaping. Then they could be replaced after (before md parsing). This _may_ be good I cannot tell.
...
- ideally if we somehow knew exactly where svelte blocks containing `{` were, we could escape every other `{`, thus not needing any custom escaping logic. Unfortunately the only 100% true way to identify logic blocks is with the svelte parser, which we need to escape _before_. We can't use the svelte parser to escape before the svelte parser. Perhaps there is a genius way to solve this but I can't think of any.
...
- manual escaping for `{` and `<` _before_ svelte-parse. This could be automatic if svelte things were reliably detectable. (As mentioned above, I don't have a good way to do this.)
...
# parser idea: final (hopefully)

I'm fairly certain the original idea should work perfectly with a bit of tweaking.
New idea is to avoid svelte parse at all, only use markdown parse and heal the changes (just smarter about it tho)

Theoretically all we have to do is:

- fix things like logic blocks getting wrapped in paragraphs if there's a newline. Generally logic blocks should not be wrapped in a p at all. The easiest way to prevent this is to have them on separate lines I think, and change the ast type to raw
- fix svelte things getting html entity escaped by md parser. Easiest way is to reparse the html entities to character. Wait but we can't blanket parse because things like code blocks get escaped...hmm

Steps:

1. possibly store & placehold all escape sequences (which may be manually written by the user)
2. prevent paragraph wrapping

- find logic blocks and remove or placehold them (or fix newlines) - perhaps through a custom ast node.
- js attributes can have blank lines. Possibly find them and placehold, or do nothing about it and have the user fix it.
- probably easiest done with string manipulation before ast parsing. Adjust newlines so they are on separate lines or all on same line/no gap

3. parse html entities

- this is a blanket parse of all html entities that md parsing will add.
...
- instead I could use svelte parse again, but first escape things in brackets so that they work, and then restore them in the svast. Basically the same thing but with svast instead. Then connect text + html and parse as markdown.
- issue is that it's still unclear how to separate the html and markdown. perhaps using comments. but then at that point might as well use a custom thing

Issues:

- html is pass-through, so markdown in html doesn't get parsed
- I want to render markdown tightly with html
- can't do string stuff before (removing them) because would remove code blocks
...
1. parse mdast
2. collect text ranges that svelte can parse. (maybe just top-most node ranges if these have positional info)
